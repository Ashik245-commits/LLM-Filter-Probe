...
*   **🔍 混合探测算法 (Hybrid Algorithm)**
    *   **自适应策略**：针对长文本（>45字）采用二分查找快速收敛可疑区间；针对短文本自动切换至双向挤压算法，定位精度直达**词汇级（1-3字）**。
*   **🛡️ 智能动态优化**
    *   **三路探测**：同时探测完整、前半、后半段，有效防止因切割位置不当（如切断敏感词）导致的漏判。
    *   **全局掩码**：一旦提取出敏感词，立即在后续请求中实施全局 Mask，大幅减少重复拦截触发与 Token 消耗。
*   **🚀 极致性能体验**
...
2.  **宏观收敛**：递归二分切割，快速锁定“受污染”的文本片段。
3.  **微观定位**：遵循 "If Safe -> Stop" 原则，对短片段进行字符级剥离，精准提取核心词。
4.  **动态掩码**：提取出的敏感词将被替换为占位符，防止后续扫描重复触发，提升效率。
